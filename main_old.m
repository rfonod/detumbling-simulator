...
N_orb = 16;           % number of orbits (1h day ~ 16 orb) per simulation [-]
q0 = [1 0 0 0]';      % initial attitude (BODY wrt ECI) quaternion [q_v,q_s] [-]
w0 = 180*[1;1;1]*d2r; % initial angular velocity (BODY wrt ECI) [rad/s]
C_type   = 1;         % control type {0 - classical B-dot, 1 - weighted B-dot}
...
if MC_on % add 15% uncertainty truncated at 3sigma
...
m_rise  = .01;  % magnetorquer rise/fall time [s]
...
% Mass moment of inertia along x,y,z-axis [kg.m^2]
...
if MC_on
    I_x = I_x + truncatedGaussian(I_x/20,3*I_x/20); % 5% additional uncertainty
    I_y = I_y + truncatedGaussian(I_y/20,3*I_y/20); % 5% additional uncertainty
    I_z = I_z + truncatedGaussian(I_z/20,3*I_z/20); % 5% additional uncertainty
end
...
Ldate  = decyear(2018,8,1);     % date for IGRF 12
...
p_tmb = [1;1;1]/f_c;  % initial tumbling parameter [muT/s]
p_tmb_n = sqrt(3)*T_c;% initial tumbling parameter [muT/s]
...
xi_m  = acos(cos(inc)*cos(g_m)+sin(inc)*sin(g_m)*1); % Ref. [Avanzi & Giulietti]
k_w_T = 2*n*(1+sin(xi_m))*min(I_nom);          % B-dot gain [A.m^2.s/T]
k_w0  = k_w_T*1e6;    % B-dot gain [A.m^2.s/muT]
k_a   = 16;
eps   = (1-sqrt(3)*k_a*p_bar_l); % epsilon for the weighted B-dot [-]
no_C  = 0; %30*60;    % no-control time after orbit injection [s]
...
for k = 1:t_len
    ...
    if k>1
        ...
        inp.b_I   = b_I;
        inp.v_I   = v_I;
%        inp.T_gg  = 3*mu*cross(r_I/norm(r_I),I*norm(r_I))/R^3;
        [q, w]    = propag_att(tspan,[q; w],inp,att_solv);
        %T_max(k-1,:)=[max(abs(cross(inp.m_res,b_B*1e-6))),max(abs(dist_aer(inp.v_I,q))), max(abs(inp.T_gg))];
    end
    ...
    if exist('b_B_meas_prev_norm','var')
        b_dot      = (b_B_meas - b_B_meas_prev)/T_c;
        b_dot_norm = (b_B_meas_norm - b_B_meas_prev_norm)/T_c;
        
        % Tumble parameter update 
        p_tmb   = alpha*abs(b_dot_norm)./(2*f_c) + (1-alpha)*p_tmb;
        %p_tmb_n = alpha*(norm(b_dot_norm)./(2*f_c)-p_tmb_n) + p_tmb_n;
        p_tmb_n = norm(p_tmb);
    else
        b_dot      = zeros(3,1);
    ...
    if (t(k) > no_C) && ADCS_on
        if C_type==1
            k_w   = k_w0/(k_a*p_tmb_n+eps);
            %k_w   = k_w0*diag(1./(p_tmb+eps/sqrt(3)));
        elseif C_type==0
            k_w = k_w0;
        end
...
if MC_on
    ...
    data(25:25+3*N_orb-1,1) = t_on_orb(:);
else
    toc
    all_var = who;
