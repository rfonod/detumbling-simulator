...
%--------------------------------------------------------------------------
%                             MAIN FUNCTION
%           Simplified ADCS (Detumbling mode) Simulator for Delfi-PQ
% Authors: R. Fonod and T.R. van Abswoude
% Version: 0.9.2
...
% Change track:
% 0.9.1 (R):
% - SolUnitVec outputs in [m] instead of [AU]
% - got rid of some (dangerous) global variables. Ideally 'k' should be
% avoided too.
%
% 0.9.2 (T):
% - Removed 'global' for variable 'rs' in ProjectionFunction
% - Changed the check for solar illumination surfaces from an angle 
%   approach to one similar to that of drag torque, by looking at the 
%   direction of the components of s_B. This reduces the amount of 
%   computations and removes the need for the function VecAngles.
% - Removed function VecAngles from solar_torque.
% - Removed various chunks of commented code for readability
%
% 0.9.3 (T):
% - Added J2 gravity gradient torque
%
% 0.9.4 (T):
% - Added 'saving vectors' in propag_att for plotting of the vectors of the
% torques.
%--------------------------------------------------------------------------
if nargin == 0
    tic, clc
...
global I Iinv dim CoM mu parameters k
...
AirDens = 'medium';   % air density {low, medium, high}
C_type  = 0;          % control type {0 - "classical" B-dot, 1 - weighted B-dot}
q0 = [1 0 0 0]';      % initial attitude (BODY wrt ECI) quaternion [q_v,q_s] [-]
w0 = 30*[1;1;1]*d2r;  % initial angular velocity (BODY wrt ECI) [rad/s]
no_C  = 1800;         % duration of no-control after release [s] /1800 = 30 min/
...
saveExcel = 0;        % save data to excel file for C implementation testing
...
dim = [d_x d_y d_z d_x_p d_y_p d_z_p];    % satellite dimensions
...
Sdate  = [2018,3,31];     % date for solar variations
Ldate  = decyear(Sdate);  % date for IGRF 12
T0     = 11*60*60;        % orbital release time [s]
...
switch AirDens
    case 'low'
        % TBD
        error('Solar activity parameters not defined')
    case 'medium'
        activity   = 0.5;  % [0,1] current activity
        activityA  = 0.5;  % [0,1] 81-day average activity, centered on day of interest
        magnInd    = 0.3;  % [0,1] magnetic index
    case 'high'
        % TBD
        error('Solar activity parameters not defined')
end

[s_I,d]  = SolUnitVec(Sdate,T0); % s - sun unit vector in ECI, d - distance [m]
SolCount = 0;                    % # of 1/10th of orbit passed (for rho and phiC)
...
if C_type
    p_tmb   = [1;1;1]/f_c;  % initial tumbling parameter [muT/s] - C-weigh
    p_bar_l = .065/(2*f_c); % lower tumble parameter threshold [muT/s]
    p_bar_u = .085/(2*f_c); % upper tumble parameter threshold [muT/s]
    k_a     = 16;           % weighted B-dot design paramter [-]
    eps     = (1-sqrt(3)*k_a*p_bar_l); % epsilon for the weighted B-dot [-]
else
    p_tmb = 2*[1;1;1];    % initial tumbling parameter [muT/s] - C-class
    p_bar_l = .075;       % lower tumble parameter threshold [muT/s]
    p_bar_u = .085;       % upper tumble parameter threshold [muT/s]
end
p_tmb_n = norm(p_tmb);% norm of the initial tumbling parameter [muT/s]
C_det = 0;            % detumbled counter [-]
C_tum = 0;            % tumbling counter [-]
t_bar_det = 1*30*60;  % confirmation time for detumbled state [s]
...
k_w0 = 2*n*(1+sin(xi_m))*min(I_nom)*1e6;          % B-dot gain in [A.m^2.s/muT]
...
ADCS_on  = 1;                   % initial ADCS mode {1 - actuating, 0 - sensing}
...
if ~MC_on
    ...
    Q = zeros(4,t_len); P_tmb_n = P_tmb(1,:);
end
...
for k = 1:t_len
    ...
    if exist('b_B_meas_prev_norm','var')
        ...
        if C_type
            p_tmb = alpha*abs(b_dot_norm)./(2*f_c) + (1-alpha)*p_tmb; % C-weigh
        else
            p_tmb = alpha*abs(b_dot_norm) + (1-alpha)*p_tmb;          % C-class
        end
        p_tmb_n = norm(p_tmb);
    else
        b_dot      = zeros(3,1);
    ...
    if (t(k) > no_C) && ADCS_on
        if C_type
            k_w = k_w0/(k_a*p_tmb_n+eps); % weighted Bdot gain
        else
            k_w = k_w0;
        end
    ...
    % Update solar activity influenced parameters approximately every minute and at start
    if t(k) >= (SolCount)*T_orb/90 || k == 1
        SolCount = SolCount + 1;
        parameters = SolarActivity(r_I,Sdate,activity,activityA,magnInd,t(k));
    end
    
    % Update solar vector and distance every 5 orbits and at start
    if t(k) >= 5*(k_orb+1)*T_orb || k == 1
        [s_I,d] = SolUnitVec(Sdate,t(k)); % s - sun unit vector in ECI, d - distance [m]
    end
